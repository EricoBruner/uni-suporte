...
export default function CancelSupportModal({
  isOpen,
  closeModal,
  supportId,
}: any) {
  ...
  const refuseSupportRequest = (e: any) => {
    ...
    const updatedSupportRequest = MYSUPPORTS.map((s) => {
      if (s.id === supportId) {
        return { ...s, status: "canceled", justification: message };
      }
      return s;
    });
  ...
  return (
    <Modal isOpen={isOpen} onRequestClose={closeModal} style={customStyles}>
      <SCContentForm onSubmit={refuseSupportRequest}>
        ...
        />
        <SCButtonGroup>
          <SCSendButton type="submit">Prosseguir</SCSendButton>
          <SCCancelButton onClick={closeModal}>Voltar</SCCancelButton>
        </SCButtonGroup>
      </SCContentForm>
    </Modal>
  );
